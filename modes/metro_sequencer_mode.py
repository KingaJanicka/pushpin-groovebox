...
# ---------------------------------------------------------------------------
# Module-level menu-item data -- replaces the three ~600-line inline literals
# that were embedded inside initialize().
# ---------------------------------------------------------------------------

_SCALE_ITEMS = [
    {
        "$type": "menu-item",
        "label": "{n}/32".format(n=n),
        "onselect": {"$type": "message", "address": "/", "value": n},
    }
    for n in range(1, 33)
]

# Seq Steps / Main Steps: 2/32 .. 32/32, then 1+1/32 .. 1+32/32.
_STEP_ITEMS = [
    {
        "$type": "menu-item",
        "label": "{n}/32".format(n=n),
    ...
    {
        "$type": "menu-item",
        "label": "1+{n}/32".format(n=n),
        "onselect": {"$type": "message", "address": "/", "value": 32 + n},
    }
    for n in range(1, 33)
]


def _make_track_controls(color_func):
    """Build the 8 scale-edit controls for one instrument/track.

    Index map (relied upon by refresh_sequencer_params):
      0 = Gate Len (OSCControl range 0-1)
      1-3 = spacers
      4 = Seq Time Scale (menu, 1-32 steps)
      5 = Seq Steps     (menu, 2-64 steps)
      6 = Main Time Scale (menu, 1-32 steps), default value 2
      7 = Main Steps    (menu, 2-64 steps), default value 64
    """
    gate_len = OSCControl(
        {"$type": "control-range", "label": "Gate Len", "address": "/", "min": 0, "max": 1},
        color_func, None,
    )
    seq_time_scale = OSCControlMenu(
        {"$type": "control-menu", "menu_label": "Seq Time Scale", "items": _SCALE_ITEMS},
        color_func, None,
    )
    seq_steps = OSCControlMenu(
        {"$type": "control-menu", "menu_label": "Seq Steps", "items": _STEP_ITEMS},
        color_func, None,
    )
    main_time_scale = OSCControlMenu(
        {"$type": "control-menu", "menu_label": "Main Time Scale", "items": _SCALE_ITEMS},
        color_func, None,
    )
    main_time_scale.value = 2

    main_steps = OSCControlMenu(
        {"$type": "control-menu", "menu_label": "Main Steps", "items": _STEP_ITEMS},
        color_func, None,
    )
    main_steps.value = 64

    return [gate_len, ControlSpacer(), ControlSpacer(), ControlSpacer(),
            seq_time_scale, seq_steps, main_time_scale, main_steps]


class MetroSequencerMode(MelodicMode):
    # Immutable configuration -- safe to keep at class scope.
    ...
    def initialize(self, settings):
        # -- instance-level mutable state (was class-level, caused cross-test bleed) --
        ...
        self.selected_track = TRACK_NAMES_METRO[0]
        self.pads_press_time = [False] * 64
        self.disable_controls = False
        self.draw_pads = True
        self.instrument_scale_edit_controls = {}
        self.scale_edit_controls = []
        self.metro_seq_pad_state = {}
        self.steps_held = []
        self.encoders_held = [False] * 8
        self.encoder_incr_since_held = [False] * 8
        self.rachets = {}
        ...
        for instrument_short_name in self.get_all_distinct_instrument_short_names_helper():
            self._init_instrument(instrument_short_name)

    @staticmethod
    def _default_pad_state():
        """Return a fresh 8x8 pad state grid (bottom row active, rest off)."""
        state = [[False] * 8 for _ in range(7)]
        state.append([True] * 8)
        return state

    def _init_instrument(self, instrument_short_name):
        """Initialise all per-instrument state for one instrument short name."""
        ...
        self.rachets[instrument_short_name] = [False] * 64
        self.instrument_scale_edit_controls[instrument_short_name] = (
            _make_track_controls(self.get_current_instrument_color_helper)
        )
        self.metro_seq_pad_state[instrument_short_name] = {
            track_name: self._default_pad_state()

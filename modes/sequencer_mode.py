...
class SequencerMode(MelodicMode):
    ...
    async def sequencer_on_tick(self, instrument_name, length):
        # if seq is active
        if await self.app.is_mode_active(self.app.sequencer_mode):
    ...
    async def update_pads(self):
        try:
            ...
            seq_pad_state = await seq.get_track(self.selected_track)
            ...
            for i, value in enumerate(await seq.get_track(self.selected_track)):
    ...
    async def on_button_pressed(self, button_name):
        ...
        if button_name in track_button_names:
            ...
            await super().on_button_pressed(button_name)
                ...
                if len(self.app.steps_held) != 0:
                    for mode in self.app.active_modes:
                        if mode == self.app.trig_edit_mode:
                            ...
                            if await seq.get_lock_state(idx, encoder_idx) is None:
                                ...
                                lock_value = await seq.get_lock_state(idx, encoder_idx)
                            ...
                            await seq.set_lock_state(idx, encoder_idx, value)
                            return
                if self.show_scale_menu == True:

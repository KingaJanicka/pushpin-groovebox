...
class MIDICCMode(PyshaMode):
    ...
    def initialize(self, settings=None):
        for (
            ...
            if midi_cc is not None:
                ...
                print(
                    "Loaded {0} MIDI cc mappings for instrument {1}".format(
                        len(self.instrument_midi_control_ccs[instrument_short_name]),
                        instrument_short_name,
                    )
                )
            else:
                ...
                print(
                    "Loaded default MIDI cc mappings for instrument {0}".format(
                        instrument_short_name
                    )
                )
        ...
        for instrument_short_name in self.instrument_midi_control_ccs:
            self.current_selected_section_and_page[instrument_short_name] = (
                self.instrument_midi_control_ccs[instrument_short_name][0].section,
                0,
            )
    ...
    def deactivate(self):
        for button_name in self.midi_cc_button_names + [
    ...
    def update_buttons(self):
        ...
        show_prev, show_next = (
            self.get_should_show_midi_cc_next_prev_pages_for_section()
        )
        if show_prev:

...
class PresetSelectionMode(definitions.PyshaMode):
    ...
    async def save_pad_to_state(self):
    ...
    async def load_init_state(self, instrument_shortname):
        ...
        await asyncio.sleep(0.5)
        await instrument.query_all_controls()
    ...
    async def create_dict_from_paths(self, arr):
        d = dict()
        for path in arr:
            parent = d
    ...
    async def new_instrument_selected(self):
        self.current_page = 0
        await self.save_all_presets_to_state()
    ...
    async def get_preset_path_for_instrument(self, instrument_shortname):
        ...
        path = await self.get_preset_path(preset)
        return preset
    ...
    async def preset_num_in_favourites(self, preset_number, bank_number):
        instrument_short_name = (
            self.app.instrument_selection_mode.get_current_instrument_short_name()
        )
        if instrument_short_name not in self.presets:
    ...
    async def get_num_pages(self):
        ...
        return await self.get_num_banks() * 2

    async def next_page(self):
        if self.current_page < await self.get_num_pages() - 1:
            self.current_page += 1
        else:
            self.current_page = await self.get_num_pages() - 1
        self.app.pads_need_update = True
        self.app.buttons_need_update = True
        await self.notify_status_in_display()
    ...
    async def has_prev_next_pages(self):
        has_next = False
        has_prev = False
        if await self.get_current_page() < await self.get_num_pages() - 1:
            has_next = True
        if await self.get_current_page() > 0:
            has_prev = True
        return (has_prev, has_next)

    async def pad_ij_to_bank_and_preset_num(self, pad_ij):
        preset_num = (await self.get_current_page() % 2) * 64 + pad_ij[0] * 8 + pad_ij[1]
        bank_num = await self.get_current_page() // 2
    ...
    async def notify_status_in_display(self):
        bank_number = await self.get_current_page() // 2 + 1
        bank_names = await self.get_bank_names()
        ...
        await self.app.add_display_notification(
            "Preset: bank {0}, presets {1}".format(
                bank_name, "1-64" if await self.get_current_page() % 2 == 0 else "65-128"
            )
        )
    ...
    async def update_buttons(self):
        show_prev, show_next = await self.has_prev_next_pages()
        if show_prev:
            self.push.buttons.set_button_color(
    ...
    async def on_pad_pressed(self, pad_n, pad_ij, velocity):
        ...
        await self.send_osc("/patch/load", self.presets[instrument_short_name][pad_ij[0]])
        await self.update_pads()
    ...
    async def on_pad_released(self, pad_n, pad_ij, velocity):
        ...
        await instrument.query_all_controls()
        await instrument.update_current_devices()
        await instrument.init_devices_sync()
        await self.update_pads()
        return True  # Prevent other modes to get this event

    async def nested_draw(
        self,
        ctx,
        current,
        level,
        ...
        for idx, entry in enumerate(current.items()):
            ...
            if isinstance(val, str) and idx - 2 <= self.state[level] <= idx + 3:
                if idx == int(self.state[level]):
                    self.current_address = os.path.splitext(await self.get_preset_path(val))[
                        0
                    ]
                show_text(
                    ctx,
                ...
                if int(self.state[level]) == idx:
                    await self.nested_draw(ctx, val, level=level + 1, max_height=max_height)

    async def get_preset_path(self, preset):
        chosen_folder = None
        if 0 <= self.state[0] < 1:
    ...
    async def update_display(self, ctx, w, h):
        await self.nested_draw(ctx, self.patches, level=0, max_height=h)
        show_text(
            ctx,
            6,
            15,
    ...
    async def set_knob_postions(self):

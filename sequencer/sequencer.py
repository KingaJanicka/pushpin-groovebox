    ...
    def evaluate_and_play_notes(self):
        try:
            ...
            gate_pitch = (
                ...
                else int(instrument_state["gate_1"][0])
            )
            gate_octave = (
                int(gate_trig_menu_locks[1]) * 12
                if gate_trig_menu_locks[1] is not None
                else int(instrument_state["gate_1"][1]) * 12
            )
            gate_note = gate_pitch + gate_octave
            gate_velocity = (
                instrument_state["gate_1"][2]
                if instrument_state["gate_1"][2] is not None
                else int(instrument_state["gate_1"][2])
            )
            ...
            pitch_recur_default = self.app.trig_edit_mode.state[instrument_name][
                "pitch_1"
            ][8]
            pitch_recur = (
                int(pitch_recur_default)
                if pitch_trig_menu_locks[8] == None
                else int(pitch_trig_menu_locks[8])
            )
            pitch_recur_len = int(
            ...
            pitch_pitch = (
                ...
                if pitch_trig_menu_locks[0] is not None
                else int(instrument_state["pitch_1"][0])
            )
            pitch_octave = (
                int(pitch_trig_menu_locks[1]) * 12
                if pitch_trig_menu_locks[1] is not None
                else int(instrument_state["pitch_1"][1]) * 12
            )
            pitch_note = pitch_pitch + pitch_octave
            pitch_velocity = (
                instrument_state["pitch_1"][2]
                if instrument_state["pitch_1"][2] is not None
                else int(instrument_state["pitch_1"][2])
            )
            ...
            trig_mute_recur_default = self.app.trig_edit_mode.state[instrument_name][
                "trig_mute_1"
            ][8]
            trig_mute_recur = (
                int(trig_mute_recur_default)
                if trig_mute_trig_menu_locks[8] == None
                else int(trig_mute_trig_menu_locks[8])
            )
            ...
            accent_prob = (
                True if instrument_state["accent_1"][4] >= random.random() else False
            )
            accent_velocity = (
                instrument_state["accent_1"][2]
                if instrument_state["accent_1"][2] is not None
                else int(instrument_state["accent_1"][2])
            )
            accent_loop_count = int(self.playhead / int(accent_track_len))
            accent_recur_default = self.app.trig_edit_mode.state[instrument_name][
                "accent_1"
            ][8]
            accent_recur = (
                int(accent_recur_default)
                if accent_trig_menu_locks[8] == None
                else int(accent_trig_menu_locks[8])
            )
            ...
            if (
                self.pitch_1[pitch_step] == True
                and pitch_prob == True
                and pitch_recur_binary_list[int(pitch_loop_count % pitch_recur_len)]
                == 1
                and pitch_track_active == True
            ):
                note = pitch_note
            ...
            # Evaluate Mute track
            if (
                self.trig_mute_1[trig_mute_step] == True
                and trig_mute_prob == True
                and trig_mute_recur_binary_list[
                    int(trig_mute_loop_count % trig_mute_recur_len)
                ]
                == 1
                and trig_mute_track_active == True
            ):
                schedule_note = False

            # Evaluate Accent track
            if (
                self.accent_1[accent_step] == True
                and accent_prob == True
                and accent_recur_binary_list[int(accent_loop_count % accent_recur_len)]
                == 1
                and accent_track_active == True
            ):

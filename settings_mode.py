...
class SettingsMode(definitions.PyshaMode):
    ...
    def check_for_delayed_actions(self):
        ...
        if self.app.midi_in_tmp_device_idx is not None:
            ...
            if (
                current_time
                - self.encoders_state[push2_python.constants.ENCODER_TRACK1_ENCODER][
        ...
        if self.app.midi_out_tmp_device_idx is not None:
            ...
            if (
                current_time
                - self.encoders_state[push2_python.constants.ENCODER_TRACK3_ENCODER][
                    "last_message_received"
                ]
                > definitions.DELAYED_ACTIONS_APPLY_TIME
            ):
        ...
        if self.app.notes_midi_in_tmp_device_idx is not None:
            ...
            if (
                current_time
                - self.encoders_state[push2_python.constants.ENCODER_TRACK6_ENCODER][
                    "last_message_received"
                ]
                > definitions.DELAYED_ACTIONS_APPLY_TIME
            ):
                self.app.set_notes_midi_in_device_by_index(
    ...
    def update_buttons(self):
        if self.current_page == 0:  # Performance settings
            ...
            self.push.buttons.set_button_color(
                push2_python.constants.BUTTON_UPPER_ROW_1, definitions.GREEN
            )
            self.push.buttons.set_button_color(
                push2_python.constants.BUTTON_UPPER_ROW_2, definitions.OFF_BTN_COLOR
            )
    ...
    def update_display(self, ctx, w, h):
        ...
        for i in range(0, 8):
            ...
            ctx.rectangle(
                part_x - 3, part_y, w, h
            )  # do x -3 to add some margin between parts
            ctx.fill()
            ...
            if self.current_page == 0:  # Performance settings
                ...
                if i == 0:  # MIDI in device
                    ...
                    show_value(
                        ...
                        h,
                        (
                            self.app.midi_in_channel + 1
                            if self.app.midi_in_channel > -1
                            else "All"
                        ),
                        color,
                    )
                    ...
                    show_value(
                        ...
                        h,
                        (
                            self.app.midi_out_channel + 1
                            if self.app.midi_out_channel >= 0
                            else "TR"
                        ),
                        color,
                    )
                    ...
                    # show_value(ctx, part_x, h, self.app.instrument_selection_mode.pyramidi_channel + 1, color)
                    ...
                    i == 6
                ):  # Re-send MIDI connection established (to push, not MIDI in/out device)
    ...
    def on_encoder_rotated(self, encoder_name, increment):
        ...
        if self.current_page == 0:  # Performance settings
            ...
            if encoder_name == push2_python.constants.ENCODER_TRACK1_ENCODER:
                ...
                if self.app.midi_in_tmp_device_idx >= len(
                    self.app.available_midi_in_device_names
                ):
                    self.app.midi_in_tmp_device_idx = (
                        len(self.app.available_midi_in_device_names) - 1
                    )
                ...
                if self.app.midi_out_tmp_device_idx >= len(
                    self.app.available_midi_out_device_names
                ):
                    self.app.midi_out_tmp_device_idx = (
                        len(self.app.available_midi_out_device_names) - 1
                    )
                ...
                pass
                # self.app.instrument_selection_mode.set_pyramidi_channel(
                #     self.app.instrument_selection_mode.pyramidi_channel + increment,
                #     wrap=False,
                # )
                ...
                if self.app.notes_midi_in_tmp_device_idx >= len(
                    self.app.available_midi_in_device_names
                ):
                    self.app.notes_midi_in_tmp_device_idx = (
                        len(self.app.available_midi_in_device_names) - 1
    ...
    def on_button_pressed(self, button_name):
        ...
        if self.current_page == 0:  # Performance settings
            ...
            if button_name == push2_python.constants.BUTTON_UPPER_ROW_1:
                ...
                # self.app.instrument_selection_mode.set_pyramidi_channel(
                #     self.app.instrument_selection_mode.pyramidi_channel + 1, wrap=False
                # )
                return True
